...
Right-side drawer. Slides in over the page, full viewport height, ~420px wide on desktop, full-width on mobile. Backdrop click dismisses. Matches the existing dark design system exactly.
...
Open `app/globals.css` and add this keyframe inside the existing CSS (after the last `@keyframes` block, or at the end of the file):
...
```bash
git add components/AIExplanationPanel.tsx app/globals.css
git commit -m "feat: add AIExplanationPanel as right-side drawer"
```
