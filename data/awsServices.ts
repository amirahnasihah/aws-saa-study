...
export const domains: DomainData[] = [
      ...
      },
      {
        ...
        services: [
          ...
          },
          {
            ...
            perangkap: [{"soalan": "EC2 dalam private subnet tak terima traffic. Kau perlu tahu sama ada SG atau NACL yang sekat, tanpa jejas prestasi. Apa kau enable?", "umpan": "Traffic Mirroring untuk capture packet dan lihat apa berlaku. Nampak betul sebab 'capture traffic'. SALAH: Traffic Mirroring salin full packet (mahal + overhead) — terlebih untuk sekadar tahu ACCEPT/REJECT.", "betul": "VPC Flow Logs — cari record action = REJECT. Keyword 'rule mana yang block / audit flow / tanpa overhead' = Flow Logs (metadata)."}, {"soalan": "Pasukan security nak inspect isi sebenar packet (payload) untuk IDS/IPS forensics dalam VPC. Guna apa?", "umpan": "VPC Flow Logs, hantar ke OpenSearch untuk analisa. Nampak betul sebab Flow Logs pun untuk security analysis. SALAH: Flow Logs metadata je — takde payload, tak boleh buat deep packet inspection.", "betul": "Traffic Mirroring (copy ke monitoring appliance/NLB). Keyword 'packet content/payload/deep inspection' = Traffic Mirroring, bukan Flow Logs."}],
            diagram: {
  ...
  FM -.->|"auto-cover"| NEW["🆕 Account baru<br/>auto patuh ✅"]`,
          ...
          },
          {
            ...
            sifir: ["8 service pre-authorized (NO approval): EC2, RDS, Aurora, CloudFront, API Gateway, Lambda, Lightsail, Elastic Beanstalk", "DILARANG walau atas resource sendiri: DoS/DDoS simulation, port/protocol/request flooding, DNS zone walking", "Bukan 'kena minta approval dulu', bukan 'boleh test semua' — 8 service spesifik", "Test resource SENDIRI je, BUKAN AWS underlying infra / tenant lain", "DDoS simulation perlu engage AWS DDoS Simulation Testing program berasingan", "AUP (Acceptable Use Policy) = dokumen tetap apa boleh/tak boleh"],
            perangkap: [{"soalan": "Apa pendirian (position) AWS terhadap penetration testing oleh pelanggan?", "umpan": "AWS tak benarkan pentest langsung sebab infra dikongsi. Nampak masuk akal sebab 'jaga tenant lain'. SALAH: AWS memang benarkan pentest pada service tertentu.", "betul": "AWS benarkan pentest pada SETENGAH resource (8 service) TANPA kelulusan awal — bukan semua, bukan tiada. Keyword '8 permitted services / no prior authorization'."}, {"soalan": "Security team nak jalankan pentest pada EC2 & RDS production. Apa langkah PERTAMA yang diperlukan?", "umpan": "Buka AWS Support case / minta kelulusan AWS dulu sebelum mula. Nampak betul sebab 'ini infra AWS, mesti kena izin'. SALAH: AWS dah PRE-AUTHORIZE 8 service ni termasuk EC2 & RDS — tak perlu approval.", "betul": "Terus pentest — EC2 & RDS antara 8 service pre-authorized. Keyword 'pentest 8 permitted services' = no prior approval needed."}, {"soalan": "Pasukan nak uji ketahanan app dengan simulate DDoS volumetrik pada ALB mereka sendiri. Boleh terus buat?", "umpan": "Boleh, sebab ia resource sendiri dan pentest dibenarkan. Nampak betul sebab 'resource saya'. SALAH: DoS/DDoS simulation DILARANG di bawah AUP walaupun atas resource sendiri.", "betul": "Tak boleh terus — DDoS simulation prohibited; kena engage AWS DDoS Simulation Testing program berasingan. Keyword 'DDoS/flooding' = prohibited activity."}],
            contohGuna: 'Security team nak test EC2 instances atau RDS databases untuk vulnerabilities — dibenarkan tanpa minta izin AWS terlebih dahulu. Tapi nak uji DDoS resilience → engage AWS DDoS Simulation Testing program dulu.',
            detailsLabel: 'Pentest policy — 3 kategori aktiviti',
            storageDetails: '🟢 Pre-authorized (NO approval) → 8 service: EC2, RDS, Aurora, CloudFront, API Gateway, Lambda (+ Lambda@Edge), Lightsail, Elastic Beanstalk. Terus test resource sendiri.\n🔴 Prohibited (haram, walau resource sendiri) → DoS/DDoS simulation, port flooding, protocol flooding, request flooding, DNS zone walking via Route 53.\n🟡 Needs separate program → DDoS / network stress / simulated event testing: kena engage AWS DDoS Simulation Testing program / dapat kebenaran khas dulu.',
            scenario: '"AWS Acceptable Use Policy", "penetration testing position", "security assessments on AWS" → AWS allow pentest on SOME resources WITHOUT prior authorization. Bukan semua resources, bukan tiada langsung — 8 services spesifik. "DDoS/stress simulation" → prohibited under AUP, perlu AWS DDoS Simulation Testing program. Keyword "request AWS approval first / open support case" untuk 8 service ni → SALAH (dah pre-authorized).',
            compare: {
              label: 'Pentest — boleh terus, haram, atau perlu program lain?',
              headers: ['Aktiviti', 'Status', 'Tindakan'],
              rows: [
                ['Vuln scan / pentest EC2, RDS, Aurora, CloudFront, API GW, Lambda, Lightsail, Beanstalk', '🟢 Pre-authorized', 'Terus buat — no approval'],
                ['Pentest service LUAR 8 senarai tu', '🟡 Semak', 'Rujuk AWS / minta kebenaran dulu'],
                ['DoS / DDoS / flooding simulation', '🔴 Prohibited', 'Engage AWS DDoS Simulation Testing program'],
                ['DNS zone walking (Route 53)', '🔴 Prohibited', 'Tak dibenarkan langsung'],
                ['Test infra AWS underlying / tenant lain', '🔴 Prohibited', 'Tak dibenarkan langsung'],
              ],
              takeaway: 'Tiga kategori: 🟢 8 service = terus pentest tanpa approval; 🔴 DoS/DDoS/flooding/zone-walking = haram walau atas resource sendiri; 🟡 nak uji DDoS resilience = engage AWS DDoS Simulation Testing program. Exam trap: "kena minta approval AWS dulu untuk 8 service" = SALAH; "boleh DDoS sebab resource sendiri" = SALAH.',
            },
            mermaid: {
              label: 'Decision tree — boleh terus pentest ke tak?',
              source: `flowchart TD
  Q["Nak pentest apa?"] --> S{"Antara 8 pre-authorized service?<br/>EC2·RDS·Aurora·CloudFront·<br/>API GW·Lambda·Lightsail·Beanstalk"}
  S -->|"Ya"| T{"Aktiviti DoS/DDoS/<br/>flooding/zone-walking?"}
  S -->|"Tidak"| ASK["🟡 Rujuk AWS / minta<br/>kebenaran dulu"]
  T -->|"Tidak"| GO["🟢 Terus pentest<br/>NO approval needed"]
  T -->|"Ya (DDoS/flooding)"| PROG["🔴 Prohibited di AUP<br/>→ engage AWS DDoS<br/>Simulation Testing program"]`,
              caption: 'INGAT exam: dua syarat untuk "terus pentest tanpa izin" — (1) salah satu 8 service pre-authorized, DAN (2) bukan aktiviti DoS/DDoS/flooding/zone-walking. Gagal mana-mana satu → tak boleh terus. "request approval first" untuk 8 service tu = jawapan SALAH.',
            },
            tips: [
  ...
  F -->|Langsung tak nak urus| I[AWS Owned Key<br/>free, tak boleh audit]`,
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Redshift cluster sedia ada tak encrypted. Audit mahu data AT REST di-encrypt. Apa kau buat?", "umpan": "Pindahkan cluster ke private subnet supaya tak terdedah. Nampak betul sebab 'lebih selamat'. SALAH: private subnet = network isolation, bukan encryption — data on-disk masih plaintext.", "betul": "Enable encryption guna KMS (AES-256) untuk data at rest. Keyword 'encrypt at rest / data on disk' = KMS, bukan network/subnet."}, {"soalan": "Team analytics dalam account lain perlu query data dalam Redshift kau secara live, tanpa salin atau pipeline. Cara mana?", "umpan": "Export data ke S3, copy ke account lain, COPY masuk Redshift mereka. Nampak betul sebab 'mereka dapat data'. SALAH: itu duplication + ETL, data jadi basi & mahal — bercanggah dengan 'tanpa salin/live'.", "betul": "Redshift DataShare — cross-account share data live tanpa ETL/duplication. Keyword 'share cross-account, no ETL, no copy' = DataShare."}],
  ...
  B -->|Resource compliant?<br/>apa config berubah?| E[AWS Config<br/>config history + drift]`,
          ...
          },
          {
            ...
            perangkap: [
              ...
              {
                soalan: 'Cert ACM kat ALB akan expire 13 bulan lagi, asalnya di-validate guna EMAIL. Status sekarang "Pending validation". Manager nak pastikan cert tak expire mengejut. Punca + fix: (A) cert hampir expire, perlu respond email validation manual sebelum ACM boleh renew, (B) cert dah expired, tak boleh renew — request cert baru, (C) ACM akan auto-renew nanti, takde action perlu, (D) cert dah expired, contact AWS Support renew manual.',
        ...
        services: [
          {
            ...
            perangkap: [{"soalan": "Web server kena boleh diakses dari internet, tapi database TAK boleh langsung. Macam mana susun dalam VPC?", "umpan": "Letak web server & DB dalam dua VPC berasingan supaya DB terpencil. Nampak betul sebab 'asingkan DB'. SALAH: terlebih kompleks — dalam satu VPC pun boleh isolate guna subnet + route.", "betul": "Satu VPC: web server dalam public subnet (route ke IGW), DB dalam private subnet (takde route ke internet). Keyword 'tak boleh diakses dari internet' = private subnet."}, {"soalan": "Kau nak satu subnet bentang merentas dua AZ untuk high availability. Boleh?", "umpan": "Boleh, buat satu subnet besar yang span 2 AZ. Nampak betul sebab 'subnet besar = lebih HA'. SALAH: subnet TERIKAT pada satu AZ sahaja.", "betul": "Tak boleh — satu subnet = satu AZ. Untuk HA, buat subnet berasingan dalam setiap AZ. Keyword 'subnet span satu AZ je'."}],
      ...
      },
      {
        ...
        services: [
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Perlu recovery laju (RTO minit), full stack dah berjalan supaya bencana cuma perlu scale up + failover, tapi tak nak bayar 2× infra penuh. Pola mana?", "umpan": "Multi-Site Active/Active — sebab dia paling laju recover. Nampak betul sebab 'recovery laju'. SALAH: Multi-Site bayar 2× full capacity, terlebih mahal untuk keperluan ni.", "betul": "Warm Standby — full stack scaled-DOWN, sentiasa ON, scale up bila failover. Keyword 'full stack jalan kecil / scale up / tak nak 2× penuh' = Warm Standby."}, {"soalan": "Apa beza utama Warm Standby dengan Pilot Light?", "umpan": "Warm Standby DB lebih besar, Pilot Light DB lebih kecil. Nampak betul sebab 'standby = lebih banyak'. SALAH: beza utama bukan saiz DB — ia status app server.", "betul": "Warm Standby app server ON (scaled-down, melayan traffic kecil); Pilot Light app server OFF. Keyword 'app ON vs OFF'."}],
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Banking app — downtime walau beberapa minit pun tak boleh diterima, RPO near-zero, RTO saat. Pola DR mana?", "umpan": "Warm Standby + Route 53 failover — sebab dia cepat recover. Nampak betul sebab 'cepat'. SALAH: Warm Standby masih kena SCALE UP dulu (RTO minit), bukan saat → ada downtime.", "betul": "Multi-Site Active/Active — dua region full capacity LIVE serentak, region fail terus serap traffic (RTO saat). Keyword 'zero downtime / RTO saat / mission-critical' = Multi-Site."}, {"soalan": "App dalaman, downtime sejam pun OK, nak jimat kos. Engineer cadang Multi-Site Active/Active untuk 'lebih selamat'. Sesuai?", "umpan": "Sesuai — lebih reliable, kenapa tak. Nampak betul sebab 'paling selamat'. SALAH: Multi-Site bayar 2× infra penuh — membazir bila RTO/RPO longgar.", "betul": "Tak sesuai — guna Pilot Light atau Warm Standby yang lebih murah. Multi-Site HANYA untuk mission-critical/zero-downtime. Keyword 'RTO/RPO longgar → strategi murah'."}],
      ...
      },
      {
        ...
        services: [
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Nak transfer 80TB ke S3 tapi internet link company lambat + ambil berbulan kalau upload online. Pilih apa?", "umpan": "AWS DataSync — DataSync laju untuk online transfer, tapi kalau network sendiri yang jadi bottleneck, online tetap berbulan.", "betul": "Snowball Edge — keyword 'limited bandwidth / would take weeks-months online / petabyte-scale offline' = Snow Family."}, {"soalan": "Perlu run ML inference di lokasi terpencil tiada internet, lepas tu hantar data balik ke AWS. Pilih apa?", "umpan": "Snowball Edge Storage Optimized — banyak storage, tapi inference perlu compute (vCPU/RAM), Storage Optimized kurang power.", "betul": "Snowball Edge Compute Optimized — keyword 'edge ML inference / heavy compute disconnected' = Compute Optimized (104 vCPU)."}],
      ...
      },
      {
        ...
        services: [
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Tengah migrate 50 server (MGN) + 10 database (DMS), nak track semua progress dari SATU tempat. Pilih apa?", "umpan": "Buka console MGN dan DMS berasingan untuk tengok status — boleh tapi berselerak, bukan single view.", "betul": "AWS Migration Hub — keyword 'single dashboard / track migrations across tools' = Migration Hub."}, {"soalan": "Pasukan kata 'guna Migration Hub untuk migrate server ke EC2'. Betul ke?", "umpan": "Ya — nama 'Migration Hub' bunyi macam ia yang buat migration, ini perangkap utama.", "betul": "TIDAK — Migration Hub cuma TRACK; yang migrate server = MGN, migrate DB = DMS. Hub = dashboard sahaja."}],
  ...
  CHK -->|"Tak, pendek + event-driven"| OK["✅ Lambda sesuai<br/>(scale to zero, cold start risk)"]`,
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Run Docker containers dengan operational overhead PALING rendah, tak nak urus/patch server. Launch type mana?", "umpan": "EC2 launch type + Auto Scaling — boleh scale, tapi kau MASIH kena patch & manage EC2 host = ada ops overhead.", "betul": "Fargate — keyword 'least operational overhead / no servers to manage / serverless container' = Fargate."}, {"soalan": "Container ECS gagal pull image dari ECR dan tak boleh tulis log ke CloudWatch. IAM role mana kena betulkan?", "umpan": "Task Role — orang ingat semua akses AWS guna Task Role, tapi Task Role untuk APP code (S3/DynamoDB), bukan pull image.", "betul": "Task Execution Role — keyword 'pull image from ECR / write logs to CloudWatch' = Task Execution Role (agent), bukan Task Role."}],
  ...
  BATCH -->|"Perlu kawal OS/GPU,<br/>fleet steady murah"| E["🖥️ EC2 / ASG<br/>(kau urus)"]`,
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Migrate Docker microservice yang run berterusan (24/7) ke AWS dengan operational overhead paling minimum, tak nak urus EC2 langsung. Pilih?", "umpan": "Lambda — sebab 'serverless, no infra' & dua-dua 'serverless' jadi orang campur.", "betul": "ECS/EKS on Fargate — keyword 'container + run berterusan + no EC2'. Lambda = FUNCTION event-driven max 15 min, bukan untuk container long-running 24/7."}, {"soalan": "Workload steady 24/7 besar yang perlu paling murah per unit + kawalan host (GPU). Pilih?", "umpan": "Fargate — sebab 'no server management' nampak senang & moden.", "betul": "ECS/EKS on EC2 — keyword 'cheapest at steady scale + host-level/GPU control'. Fargate lagi mahal per unit & tiada akses host; EC2 (+RI/SP) lebih murah untuk fleet steady."}],
            diagram: {
          ...
          },
          {
            ...
            perangkap: [{"soalan": "HPC workload perlu jutaan IOPS untuk scratch/shuffle space, data sementara boleh dibina semula kalau hilang. Pilih?", "umpan": "EBS io2 — sebab 'IOPS tinggi' terus terfikir io2 Provisioned IOPS.", "betul": "Instance Store — keyword 'jutaan IOPS + scratch + data boleh hilang'. io2 max 64K IOPS & melalui network; Instance Store fizikal = jutaan IOPS, percuma, sesuai untuk scratch."}, {"soalan": "Database storage MESTI survive walaupun instance stop atau host fail. Pilih?", "umpan": "Instance Store — sebab 'laju' nampak bagus untuk database.", "betul": "EBS — keyword 'survive stop/failure (persistent)'. Instance Store EPHEMERAL: stop/terminate/host fail = data hilang. Database perlu persistent = EBS."}],
          ...
          },
          {
            ...
            perangkap: [{"soalan": "HPC cluster jalankan MPI distributed ML training, perlu latency paling rendah antara node yang tightly-coupled. Pilih adapter?", "umpan": "ENA — sebab 'high performance network' nampak macam jawapan paling laju.", "betul": "EFA (Elastic Fabric Adapter) — keyword 'lowest latency + MPI + tightly-coupled HPC'. ENA laju tapi tiada OS-bypass; hanya EFA bagi OS-bypass untuk latency paling rendah inter-node."}, {"soalan": "EC2 di subnet failover — bila instance utama mati, alamat IP private mesti pindah cepat ke instance standby. Guna apa?", "umpan": "Tukar IP guna ENA — sebab orang ingat ENA = 'network adapter' jadi boleh urus IP.", "betul": "ENI (Elastic Network Interface) — detach ENI dari instance mati, attach ke standby = IP private ikut sama. ENA cuma driver throughput, bukan untuk pindah IP."}],
    ...
    Q2 -->|"Ya — tightly-coupled"| EFA["🚀 EFA — Fabric<br/>Kereta Formula 1 (litar HPC)<br/>ENA + OS-bypass (libfabric/MPI)<br/>Linux only + Cluster Placement Group<br/>📌 'HPC, ultra-low latency, OS bypass'"]`,
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Nak run custom code di CloudFront edge yang perlu query DynamoDB untuk authorize setiap request. Pilih?", "umpan": "CloudFront Functions — sebab 'code at edge' & lebih murah/laju nampak menarik.", "betul": "Lambda@Edge — keyword 'call DynamoDB / AWS service at edge'. CloudFront Functions tiada network access & tak boleh panggil AWS service; hanya Lambda@Edge boleh."}, {"soalan": "Simple rewrite header / redirect URL di edge, sub-millisecond, tiada panggilan AWS, nak paling murah. Pilih?", "umpan": "Lambda@Edge — sebab 'run code at edge' biasa terus terfikir Lambda@Edge.", "betul": "CloudFront Functions — keyword 'simple header/URL rewrite + sub-ms + no AWS call + cheapest'. Lambda@Edge overkill & lagi mahal untuk transform ringkas."}],
  ...
  B -->|Encrypt dulu sebelum hantar| F[Client-Side<br/>AWS tak nampak plaintext]`,
    ...
    A1 -->|"Master safety switch"| BPA["Block Public Access<br/>override semua"]`,
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Imej X-ray jarang dibuka TAPI bila doktor minta kena dapat dalam milisaat, kos rendah. Class mana?", "umpan": "Glacier Flexible Retrieval — nampak 'jarang access = Glacier' jadi pilih Flexible.", "betul": "Glacier Instant Retrieval. Flexible kena restore job (minit-jam), tak boleh ms. Keyword: 'millisecond retrieval' + 'rarely access' → Instant."}, {"soalan": "Data compliance dalam Deep Archive, audit minta retrieve 'dalam beberapa minit'. Boleh?", "umpan": "Pilih Expedited tier untuk Deep Archive — anggap semua Glacier ada Expedited.", "betul": "TAK BOLEH. Deep Archive tiada Expedited; paling laju Standard ~12 jam. Kalau perlu segera kena Glacier Flexible/Instant. Keyword: 'Deep Archive' + 'minutes' = trap."}],
  ...
  D -->|"7 tahun"| E["🗑️ Tong sampah<br/>Auto-delete<br/>(expiration)"]`,
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Bagi customer download SATU file S3 untuk 1 jam tanpa AWS account. Guna apa?", "umpan": "Buat bucket public / bagi IAM user — atau pilih CloudFront Signed URL.", "betul": "S3 Pre-Signed URL. Simple, free, expiry-based, 1 object. CloudFront Signed URL untuk private content at scale via CDN. Keyword: 'temporary' + 'one file' + 'no credentials'."}, {"soalan": "Nak hadkan akses private video by IP + global CDN delivery. Pre-Signed URL boleh?", "umpan": "Pre-Signed URL dengan IP restriction — sangka boleh letak syarat IP.", "betul": "CloudFront Signed URL/Cookie. Pre-Signed URL HANYA boleh expiry time, tak boleh restrict IP/geo. Keyword: 'IP restriction' + 'CDN' → CloudFront."}],
  ...
  FH --> FH2["tapi tunggu tong kumpul dulu<br/>(buffer ~60s/MB) → near-real-time,<br/>auto-scale, no replay"]`,
    ...
    B -->|"Tak — simple & murah"| HTTP["HTTP API<br/>(~70% murah, lower latency)"]`,
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Nak route event ke target BERBEZA berdasarkan content/field dalam event. Guna apa?", "umpan": "SNS dengan banyak subscriber — sangka SNS boleh pilih siapa terima.", "betul": "EventBridge (content-based routing via event pattern). SNS broadcast SAMA mesej ke semua subscriber, tak boleh filter by content. Keyword: 'route based on content' → EventBridge."}, {"soalan": "EC2 instance terminate → auto-trigger Lambda untuk cleanup. Service mana?", "umpan": "SNS notification ke Lambda — sangka SNS yang detect state change.", "betul": "EventBridge rule (detect EC2 state change event → trigger Lambda). SNS tak detect AWS service events. Keyword: 'AWS service state change' → EventBridge."}],
  ...
  Q -->|"Laju, banyak,<br/>ulang pun tak apa"| EXP["📲 Blast SMS = EXPRESS<br/>pukul rata laju & murah,<br/>kadang sampai 2 kali (at-least-once),<br/>siap dalam 5 minit"]`,
          ...
          },
          {
            ...
            perangkap: [{"soalan": "On-prem app guna ActiveMQ (AMQP), nak migrate ke AWS TANPA ubah code. Service mana?", "umpan": "SQS + SNS — sangka 'messaging di AWS mesti SQS/SNS'.", "betul": "Amazon MQ. SQS/SNS guna API proprietary → kena rewrite code. MQ support protokol standard untuk lift-and-shift. Keyword: 'migrate existing' + 'AMQP/MQTT' + 'no code change'."}, {"soalan": "Bina app messaging cloud-native BARU, perlu auto-scale hampir tanpa had. Amazon MQ?", "umpan": "Amazon MQ — sangka ia general-purpose messaging untuk semua.", "betul": "SQS + SNS. MQ = broker instance (skala terhad, mahal sikit). App baru → SQS/SNS serverless. Keyword: 'new app' + 'scale' → SQS/SNS, BUKAN MQ."}],
  ...
  B -->|"Setup sekali masa<br/>instance LAUNCH"| E["User Data<br/>boot script je"]`,
          ...
          },
          {
            ...
            detailsLabel: 'AWS Config — komponen utama',
            storageDetails: 'Configuration Recorder → enjin yang detect & rekod setiap perubahan config resource. Kena ON dulu, kalau OFF Config tak rekod apa-apa\nConfiguration Item (CI) → 1 snapshot point-in-time bagi SATU resource (state + relationship + metadata). Ini unit yang kau bayar $0.003 setiap satu\nConfiguration History → timeline semua CI bagi satu resource — boleh banding "semalam vs hari ni"\nDelivery Channel → saluran hantar config snapshot + history ke S3 bucket (+ notify via SNS/EventBridge)\nConfig Rule → evaluate CI lawan desired state → tanda Compliant / Non-compliant (managed atau custom Lambda)\nRemediation (SSM Automation) → auto-fix resource yang non-compliant',
            compare: {
  ...
  B -->|Custom Spark/Hadoop, kawalan penuh cluster| G[EMR<br/>big data framework]`,
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Executive nak dashboard dari data IoT dalam S3 dengan usage forecasting, operasi minimal, tiada data warehouse. Penyelesaian?", "umpan": "Load ke Redshift + bina custom ML model untuk forecast — nampak 'proper' untuk analytics + ML.", "betul": "QuickSight (connect S3 direct) + ML Insights forecasting. Keyword 'dashboard + forecast + minimal ops' = QuickSight, bukan Redshift + custom ML/SageMaker."}, {"soalan": "Setiap analyst regional hanya boleh nampak baris data region mereka dalam dashboard yang sama. Apa diperlukan?", "umpan": "QuickSight Standard edition + buat dashboard berasingan tiap region — leceh & tak scale.", "betul": "QuickSight Enterprise + row-level security. Keyword 'restrict data per user / row-level' = Enterprise edition (Standard tiada RLS)."}],
  ...
  SPICE --> ML["🚨 Lampu amaran (ML Insights)<br/>anomaly + forecast<br/>'minyak nak habis dalam 50km'"]`,
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Perlu auto-determine schema dari data dalam S3/DynamoDB dan populate Glue Data Catalog. Komponen Glue mana?", "umpan": "Glue Classifier — sebab dia kenal pasti format data, nampak macam dia yang tentukan schema.", "betul": "Glue Crawler — keyword 'discover schema + populate Data Catalog' = Crawler (orchestrator). Classifier cuma BANTU Crawler kenal format custom; Crawler yang panggil Classifier & isi catalog."}, {"soalan": "Pasukan nak transform raw CSV dalam S3 ke Parquet dengan kawalan PENUH atas Spark cluster & framework. Service?", "umpan": "AWS Glue — sebab dia memang ETL Spark, nampak terus padan.", "betul": "EMR — keyword 'full control over Spark/cluster / pilih framework' = EMR. Glue serverless, Spark config diabstrak (kawalan terhad). Kalau soalan kata 'serverless ETL, no cluster' baru Glue."}],
  ...
  DN2 -.->|"salinan pendua"| DN3`,
          ...
          },
          {
            ...
            perangkap: [{"soalan": "App perlu extract data berstruktur (nama, jumlah, tarikh) dari ribuan invois scan PDF. Servis mana?", "umpan": "Amazon Rekognition — orang ingat 'analisa imej jadi Rekognition', tapi Rekognition untuk muka/objek/scene, bukan baca teks/borang/jadual dari dokumen.", "betul": "Amazon Textract — OCR khas untuk extract teks, forms, tables dari dokumen. Keyword: 'OCR', 'scanned PDF', 'forms/tables', 'invoice'."}, {"soalan": "Bina pipeline: terima audio meeting bahasa Jepun → simpan transkrip Inggeris → analisa sentiment. Servis mana ikut turutan?", "umpan": "Guna satu servis je macam Comprehend untuk semua — Comprehend NLP sahaja, tak boleh transkrip audio atau translate.", "betul": "Transcribe (audio→teks) → Translate (JP→EN) → Comprehend (sentiment). Setiap langkah satu pre-built API. Keyword: 'transcribe + translate + sentiment pipeline'."}],
        ...
        services: [
          {
            ...
            perangkap: [{"soalan": "Startup baru launch app, tak pasti traffic, nak elak terlock komitmen. Pricing model mana mula-mula?", "umpan": "Reserved Instances 1-tahun untuk 'jimat' — nampak murah, tapi kalau traffic tak menentu / instance type berubah, RI jadi membazir sebab dah commit.", "betul": "On-Demand dulu sampai pattern stabil, baru tukar ke RI/Savings Plans. Keyword: 'unpredictable', 'short-term', 'no commitment', 'startup baru'."}],
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Database 24/7 sepanjang tahun, instance type takkan berubah, nak diskaun MAKSIMUM. Pilih apa?", "umpan": "Convertible RI — nampak 'lagi bagus sebab fleksibel', tapi Convertible diskaun KURANG; kalau config memang takkan berubah, fleksibiliti tu tak berguna dan kau rugi diskaun.", "betul": "Standard RI (3-tahun All Upfront) — diskaun paling besar (~72%) bila config tetap. Keyword: 'config takkan berubah', 'diskaun maksimum', '24/7'."}, {"soalan": "Perlu JAMINAN kapasiti EC2 dalam satu AZ tertentu untuk DR. RI jenis mana?", "umpan": "Regional RI — orang sangka semua RI reserve capacity, tapi Regional RI TIDAK bagi capacity reservation (cuma billing discount fleksibel merentas AZ).", "betul": "Zonal RI — lock satu AZ DAN dapat capacity reservation. Keyword: 'capacity reservation', 'specific AZ', 'guaranteed capacity'."}],
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Beban batch big-data yang boleh interrupt & resume, nak kos PALING rendah. Pricing mana?", "umpan": "Reserved Instances / Savings Plans — nampak 'jimat jadi pilih commit', tapi untuk batch fault-tolerant, Spot jauh lebih murah (90% vs 72%) dan interrupt tak jadi masalah.", "betul": "Spot Instances — 'fault-tolerant + lowest cost' hampir mesti Spot. Keyword: 'fault-tolerant', 'batch', 'can resume', 'lowest cost'."}, {"soalan": "Spot workload perlu SAMBUNG kerja (resume state) selepas di-interrupt, bukan start fresh. Interruption behavior mana?", "umpan": "Terminate (default) — biar default, tapi terminate buang instance terus, kerja hilang, kena start dari awal.", "betul": "Stop (request persistent/Fleet maintain) untuk kekalkan EBS, atau Hibernate untuk simpan RAM. Keyword: 'resume work after interruption', 'preserve state'."}],
  ...
  B -->|"Tengok duit dah pergi mana"| F["Cost Explorer<br/>visualize trends"]`,
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Finance nak dapat email amaran SEBELUM monthly EC2 cost cecah $1000. Servis mana?", "umpan": "Cost Explorer — orang ingat 'kos jadi Cost Explorer', tapi Cost Explorer cuma VISUALIZE/analyse past spend, tak hantar alert proaktif.", "betul": "AWS Budgets — set threshold + alert (boleh forecast) sebelum overspend. Keyword: 'alert before overspending', 'notify at threshold'."}, {"soalan": "Bila kos cecah threshold, sistem mesti AUTO stop EC2 non-prod, bukan sekadar hantar email. Apa guna?", "umpan": "Cost Anomaly Detection — nampak macam auto-respond, tapi ia detect+alert anomali, tak ambil tindakan auto-stop.", "betul": "AWS Budgets + Budget Actions — auto apply IAM/SCP atau stop EC2 bila threshold dilanggar. Keyword: 'budget actions', 'auto-stop on threshold'."}],
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Pasukan nak tengok graf service mana cost paling banyak 6 bulan lepas DAN dapat cadangan RI nak beli. Servis mana?", "umpan": "Cost & Usage Report — nampak 'cost detail jadi CUR', tapi CUR raw data mentah ke S3 yang kau kena query/olah sendiri, bukan graf siap + recommendation.", "betul": "Cost Explorer — UI interaktif dengan chart, trend, forecast + RI/SP recommendation. Keyword: 'visualize trends', 'analyze spend', 'RI recommendation'."}],
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Finance nak data billing mentah paling detail (per-resource, per-hour) untuk query custom guna SQL. Servis mana?", "umpan": "Cost Explorer — orang ingat 'analisa cost jadi Cost Explorer', tapi Cost Explorer UI siap dengan granularity terhad, bukan raw line-item untuk SQL custom.", "betul": "Cost and Usage Report (CUR) → S3 + Athena (Parquet). Keyword: 'detailed line-item', 'raw billing data', 'query with SQL', 'most granular'."}],
        ...
        services: [
          {
            ...
            perangkap: [{"soalan": "Global users complain laju lambat + bil data transfer dari S3 tinggi. Cara murahkan + cepatkan?", "umpan": "Replicate S3 bucket ke banyak region (CRR) — sebab nampak 'dekatkan data'. SALAH: CRR gandakan kos storan + kau kena urus routing; tak selesaikan caching/transfer.", "betul": "Letak CloudFront depan S3 — cache di edge, kurang origin fetch, transfer lebih murah. Keyword: 'global users + reduce latency + reduce data transfer cost'."}],
          ...
          },
          {
            ...
            perangkap: [{"soalan": "EC2 private subnet upload banyak ke S3, bil NAT Gateway tinggi. Cara hapuskan kos itu untuk S3 traffic?", "umpan": "Tambah lagi NAT Gateway / besarkan instance — sebab fikir 'throughput'. SALAH: tambah NAT = tambah kos, bukan buang.", "betul": "Pasang S3 Gateway VPC Endpoint (PERCUMA) — traffic S3 tak lalu NAT lagi. Keyword: 'private subnet → S3 + remove NAT cost'."}],
  ...
  subgraph WT["Write-Through — isi masa TULIS"]
    ...
    A2 --> CC["Update cache serentak<br/>(sentiasa fresh)"]
          ...
          },
          {
            ...
            perangkap: [{"soalan": "App baru, tak tahu corak traffic, mahu elak throttle masa spike & elak bayar capacity idle. Mode mana?", "umpan": "Provisioned + Auto Scaling — sebab nampak 'auto-scale jugak'. SALAH: auto-scaling react LAMBAT, spike tiba-tiba boleh kena throttle sebelum sempat scale.", "betul": "DynamoDB On-Demand — instant accommodate spike, bayar per request. Keyword: 'unpredictable / unknown traffic + no capacity planning'."}],
        ...
        services: [
          {
            ...
            perangkap: [{"soalan": "Pasukan deploy manual, selalu silap, tak ada cara repeat environment. Pillar/amalan mana?", "umpan": "Reliability — sebab 'silap = tak reliable'. SALAH: Reliability pasal recover dari failure & scaling, bukan proses deploy.", "betul": "Operational Excellence — IaC (CloudFormation) + CI/CD + runbooks untuk operasi konsisten. Keyword: 'automate operations, perform operations as code'."}],
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Nak rekod siapa buat apa dalam account untuk audit + kesan aktiviti mencurigakan. Pillar/amalan mana?", "umpan": "Operational Excellence (CloudWatch metrics) — sebab nampak 'monitoring'. SALAH: itu operasi/prestasi, bukan audit identiti & ancaman.", "betul": "Security pillar — detective controls: CloudTrail (audit log) + GuardDuty. Keyword: 'detective controls, least privilege, protect data'."}],
          ...
          },
          {
            ...
            perangkap: [{"soalan": "App kena terus jalan walaupun satu Availability Zone tumbang. Pillar/strategi mana?", "umpan": "Performance Efficiency (scaling) — sebab nampak 'scaling'. SALAH: scaling pasal efisiensi resource, bukan survive AZ failure.", "betul": "Reliability — Multi-AZ deployment + auto-recovery. Keyword: 'recover from failure, survive AZ outage, RTO/RPO'."}],
          ...
          },
          {
            ...
            perangkap: [{"soalan": "Static asset disajikan global lambat; nak guna resource lebih efisien & laju tanpa scale up server. Pillar/amalan?", "umpan": "Reliability — sebab nampak 'scale'. SALAH: Reliability pasal recover/failure, bukan efisiensi penyampaian.", "betul": "Performance Efficiency — caching + CDN (CloudFront) edge delivery. Keyword: 'use resources efficiently, global deployment, caching'."}],
        ...
        services: [
          {
            ...
            perangkap: [{"soalan": "(Real-world, bukan exam) Single-server app guna SQLite, nak backup automatik + restore cepat tanpa bayar RDS. Apa guna?", "umpan": "Tukar ke RDS Multi-AZ — sebab 'reliable'. Untuk side-project ia overkill + mahal ($50–300+/bulan).", "betul": "Litestream — stream WAL ke S3, restore masa crash, kos S3 sahaja. Keyword: 'SQLite continuous replication to S3, cheap backup'."}],
