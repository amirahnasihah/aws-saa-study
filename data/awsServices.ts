    ...
    sections: [
      {
        ...
        services: [
          ...
          },
          {
            shortName: 'Lambda',
            fullName: 'AWS Lambda',
            ingat: '"Jalankan code, bayar per run — Handler → Execution Environment → Triggers"',
            gunaUntuk: 'Serverless event-driven compute — run code tanpa manage servers',
            fungsi: 'Upload function code, Lambda auto-provision execution environment, run handler bila triggered by events (S3, API Gateway, SQS, DynamoDB Streams, etc.). Bayar per request + compute duration. Max 15 min timeout, max 10GB memory.',
            contohGuna: 'Resize image bila upload ke S3, webhook handler, scheduled tasks, real-time stream processing',
            scenario: '"Java Lambda cold starts + cost-effective" → SnapStart (free, snapshot-based). "Strict sub-10ms latency for any runtime" → Provisioned Concurrency (paid, pre-warmed). "Share libraries across functions" → Lambda Layers. "Stream large responses" → Response Streaming.',
            storageDetails: 'Handler → Entry point function yang process event object. Kau tulis code, Lambda call handler setiap invocation\nExecution Environment → Isolated container yang run function. Lifecycle: INIT (cold start) → INVOKE → SHUTDOWN. Re-used for subsequent calls (warm start)\nTrigger → Event source yang invoke function: S3, API Gateway, SQS, SNS, DynamoDB Streams, EventBridge, CloudWatch Events\nEvent Source Mapping (ESM) → For stream/queue sources (SQS, Kinesis, DynamoDB Streams). Lambda poll source, batch records, invoke function\nLayer → .zip archive of shared libraries/dependencies. Max 5 layers per function. Reduce deployment package size\nConcurrency → Number of simultaneous executions. Default 1,000 per region per account. Reserved = guaranteed, Provisioned = pre-warmed\nVersions & Aliases → Version = immutable snapshot of function code+config. Alias = pointer to version (e.g. "prod" → v5). Weighted alias for canary deployments',
            detailsLabel: 'Lambda Components',
            tips: [
              'Cold Start = INIT phase (download code + initialize runtime + run init code). Java/JVM paling teruk (2-10s). Python/Node.js paling cepat (<1s)',
              'SnapStart (Java/.NET/Python 3.12+): snapshot initialized execution environment → cache → restore on invoke. Eliminates cold start. NO EXTRA COST. Reduce cold start ~90%',
              'Provisioned Concurrency: pre-warm N execution environments. Eliminates cold start for ALL runtimes. PAID — kena bayar untuk idle warm capacity 24/7',
              'SnapStart vs Provisioned Concurrency: CANNOT combine both. SnapStart = cost-effective, sub-second. Provisioned = expensive, sub-10ms. Java + cost-effective → SnapStart',
              'Lambda Layers: share dependencies across functions. Max 5 layers, total unzipped max 250MB. Bukan untuk reduce cold starts',
              'Response Streaming: stream response progressively (bukan tunggu siap baru return). Untuk large payloads. Bukan untuk cold starts',
              'Reserved Concurrency: guarantee N concurrent executions untuk function. Also CAPS maximum — functions lain tak boleh ambil reserved quota',
              'Timeout max 15 min. Memory 128MB–10GB. Deployment package max 50MB (zipped), 250MB (unzipped). /tmp storage max 10GB',
              'Exam: "Java Lambda + minimize cold start + cost-effective" → SnapStart. Bukan Provisioned Concurrency (mahal), bukan Layers (tak reduce cold start), bukan Response Streaming (unrelated)',
              'Exam: "Lambda + RDS too many connections" → RDS Proxy. "Lambda + DynamoDB changes" → DynamoDB Streams + ESM',
            ],
            docs: [
              { label: 'Lambda SnapStart', url: 'https://docs.aws.amazon.com/lambda/latest/dg/snapstart.html' },
              { label: 'Lambda Concurrency', url: 'https://docs.aws.amazon.com/lambda/latest/dg/lambda-concurrency.html' },
            ],
            keywords: ['serverless', 'event-driven', 'no server management', 'cold start', 'SnapStart', 'Provisioned Concurrency', 'Reserved Concurrency', 'Lambda Layer', 'handler', 'execution environment', 'trigger', 'event source mapping', 'ESM', '15 min timeout'],
          },
          {
          ...
          },
          {
            ...
            ingat: '"Docker manager — Cluster → Service → Task → Container"',
            gunaUntuk: 'Run dan orchestrate Docker containers pada AWS',
            fungsi: 'Fully managed container orchestration service. Kau define containers dalam Task Definition, launch sebagai Tasks, dan maintain availability melalui Services. Boleh run on EC2 (kau manage instances) atau Fargate (serverless).',
            contohGuna: 'Run microservices dalam Docker, e-commerce modules, batch processing jobs',
            scenario: '"Run containerized app with auto-recovery and load balancing" → ECS Service + ALB. "One-off batch job" → ECS Task (standalone). "No infra management" → Fargate launch type.',
            storageDetails: 'Cluster → Logical grouping of tasks/services + infrastructure (VPC, subnets). Satu cluster boleh ada banyak services\nTask Definition → Blueprint/JSON template — image, CPU, memory, ports, env vars, volumes, IAM task role. Versioned (revision numbers)\nTask → Running instance of Task Definition. Satu task = satu atau lebih containers. Standalone task = one-off job\nService → Maintains desired count of tasks. Auto-replace failed tasks. Boleh attach ALB/NLB untuk load balancing\nContainer Instance → EC2 instance yang register ke cluster + run ECS Agent (EC2 launch type sahaja)\nECS Agent → Software on EC2 yang communicate dengan ECS control plane. Fargate tak perlu — AWS manage\nCapacity Provider → Strategy untuk provision compute: Fargate, Fargate Spot, atau EC2 Auto Scaling Group',
            detailsLabel: 'ECS Components',
            tips: [
            ...
            keywords: ['Docker', 'containers', 'microservices', 'task definition', 'JSON template', 'Fargate', 'EC2 launch type', 'service', 'cluster', 'container instance', 'ECS agent', 'capacity provider'],
          },
          {
            shortName: 'EKS',
            fullName: 'Elastic Kubernetes Service',
            ingat: '"Kubernetes manager — Control Plane (AWS) → Worker Nodes (kau) → Pods"',
            gunaUntuk: 'Managed Kubernetes container orchestration pada AWS',
            fungsi: 'AWS manage Kubernetes control plane (API server, etcd, scheduler) across multiple AZs. Kau manage worker nodes (atau guna Fargate). Pods dapat real VPC IP addresses via AWS VPC CNI plugin.',
            contohGuna: 'Large-scale containerized apps yang guna K8s, multi-team microservices platform',
            scenario: '"Run Kubernetes on AWS tanpa manage control plane" → EKS. "URL path-based routing to EKS services" → AWS Load Balancer Controller + ALB + Ingress resource. "Serverless K8s pods" → EKS + Fargate.',
            storageDetails: 'Control Plane (AWS managed) → API Server (min 2 nodes, multi-AZ) + etcd (3 AZs) + scheduler + controller-manager. Endpoint guna NLB\nWorker Nodes → EC2 instances yang run kubelet + kube-proxy + container runtime. 3 types: Managed Node Groups, Self-Managed, Fargate\nPod → Smallest K8s unit. Satu atau lebih containers + shared storage + network IP. Gets real VPC IP (AWS VPC CNI)\nService → Expose pods internally (ClusterIP) atau externally (LoadBalancer/NodePort). Stable endpoint untuk group of pods\nIngress → HTTP/HTTPS routing rules (path-based, host-based). AWS Load Balancer Controller auto-provision ALB\nNamespace → Virtual cluster untuk isolate workloads (dev, staging, prod) dalam satu physical cluster\nKarpenter → Auto-provision right-sized EC2 nodes based on pod requirements. Alternative ke Cluster Autoscaler',
            detailsLabel: 'EKS Components',
            tips: [
              ...
              'Exam: "route requests to EKS by URL path with least setup" → AWS Load Balancer Controller + ALB (bukan Lambda, bukan NLB, bukan NGINX)',
            ],
            keywords: ['Kubernetes', 'K8s', 'container orchestration', 'IRSA', 'IAM Roles for Service Accounts', 'pod identity', 'control plane', 'worker nodes', 'pods', 'Ingress', 'VPC CNI', 'Karpenter'],
          },
          {
          ...
          },
          {
            ...
            ingat: '"Kubernetes Ingress → auto-provision ALB/NLB. Path-based routing = ALB, least setup"',
            gunaUntuk: 'Auto-provision AWS load balancers (ALB/NLB) dari Kubernetes Ingress/Service resources',
            fungsi: 'Kubernetes controller add-on yang watch for Ingress resources dan auto-create ALB, atau watch for Service type LoadBalancer dan auto-create NLB. Install dalam EKS cluster, integrate terus dengan AWS APIs. Kau define routing rules dalam Kubernetes YAML → controller buat semua AWS infra automatically.',
            contohGuna: 'Define Kubernetes Ingress dengan path rules (/api → service-a, /web → service-b) → Controller auto-provision ALB dengan matching target groups dan listener rules',
            scenario: '"EKS cluster perlu route requests by URL path with LEAST setup" → AWS Load Balancer Controller + ALB. Sebab: (1) ALB natively support path-based routing (Layer 7), (2) Controller auto-provision — tak perlu manual create LB. Lambda proxy = overkill + kena tulis routing logic. NLB = Layer 4, tak faham URL paths. NGINX Ingress = more setup (install + manage NGINX pods sendiri).',
            storageDetails: 'Ingress Resource → Kubernetes YAML yang define HTTP routing rules (path, host). Controller watch ni dan create ALB\nALB (auto-created) → Layer 7 load balancer dengan path/host rules. Controller create target groups + listener rules automatically\nNLB (auto-created) → For Service type LoadBalancer. Controller create NLB untuk TCP/UDP traffic\nTarget Groups → Controller register pods directly as IP targets (pod IP mode) — traffic bypass kube-proxy\nAnnotations → kubernetes.io/ingress.class: alb, alb.ingress.kubernetes.io/scheme: internet-facing, etc.',
            detailsLabel: 'LB Controller Components',
            tips: [
            ...
            docs: [
              { label: 'AWS LB Controller', url: 'https://docs.aws.amazon.com/eks/latest/userguide/aws-load-balancer-controller.html' },
              { label: 'ALB Ingress on EKS', url: 'https://docs.aws.amazon.com/eks/latest/userguide/alb-ingress.html' },
            ],
            keywords: ['AWS Load Balancer Controller', 'ALB', 'NLB', 'Ingress', 'path-based routing', 'EKS', 'Kubernetes', 'Layer 7', 'auto-provision'],
          },
          {
